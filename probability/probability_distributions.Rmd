...
This section provides the formulas for the PDF/PMF, expectation, variance, and MGF of the distributions covered.
...
### Beta Distribution
- PDF: $f(x; \alpha, \beta) = \frac{x^{\alpha- 1}(1- x)^{\beta- 1}}{B(\alpha, \beta)}$ for $0 < x < 1$
...
### Cauchy Distribution
- PDF: $f(x; x_0, \gamma) = \frac{1}{\pi\gamma[1 + (\frac{x- x_0}{\gamma})^2]}$
...
### Chi-Squared Distribution
- PDF: $f(x; k) = \frac{x^{k/2- 1}e^{- x/2}}{2^{k/2}\Gamma(k/2)}$ for $x > 0$
- Expectation: $E(X) = k$
- Variance: $Var(X) = 2k$
...
### F Distribution
- PDF: $f(x; d_1, d_2) = \frac{\sqrt{\frac{(d_1x)^{d_1}d_2^{d_2}}{(d_1x + d_2)^{d_1+d_2}}}}{xB(d_1/2, d_2/2)}$ for $x > 0$
...
- PDF: $f(x; k, \theta) = \frac{x^{k- 1}e^{- x/\theta}}{\theta^k\Gamma(k)}$ for $x > 0$
...
- PDF: $f(x; \mu, \sigma) = \frac{1}{x\sigma\sqrt{2\pi}}e^{- \frac{(\ln x -  \mu)^2}{2\sigma^2}}$ for $x > 0$
...
- PDF: $f(x; k, \lambda) = \frac{k}{\lambda}(\frac{x}{\lambda})^{k- 1}e^{- (x/\lambda)^k}$ for $x \geq 0$
- Expectation: $E(X) = \lambda\Gamma(1 + \frac{1}{k})$
- Variance: $Var(X) = \lambda^2[\Gamma(1 + \frac{2}{k}) -  (\Gamma(1 + \frac{1}{k}))^2]$
- MGF: Not available in a simple closed form.
